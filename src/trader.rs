...
impl Trader {
    ...
    pub async fn execute_buy(&self, opportunity: &BuyOpportunity) -> Result<()> {
        // Safety check: do not buy if market has ended (time_remaining 0).
        // When min_time_remaining_seconds is Some(0), allow trading until the last second; otherwise require at least that many seconds left.
        if opportunity.time_remaining_seconds == 0 {
            anyhow::bail!("Market has ended (time remaining 0). Cannot execute buy.");
        }
        let min_required = match self.config.min_time_remaining_seconds {
            Some(0) => 0u64, // Trade until closure
            Some(n) => n,
            None => 30,
        };
        if min_required > 0 && opportunity.time_remaining_seconds < min_required {
            anyhow::bail!(
                ...
                Set min_time_remaining_seconds to 0 in config to allow trading until market closure.",
