...
//! Used by the sports trailing bot (and other binaries) in LIVE mode for order signing and placement.
//! Search order: env LIBCOB_SDK_SO, then `lib/libclob_sdk.so` (Unix) or `lib/clob_sdk.dll` (Windows).
...
fn load_lib() -> Result<&'static Library> {
    let path = std::env::var("LIBCOB_SDK_SO")
        ...
        .or_else(|| {
            ...
            let candidates = ["lib/clob_sdk.dll", "clob_sdk.dll"];
            #[cfg(target_os = "macos")]
            let candidates = ["lib/libclob_sdk.dylib", "lib/libclob_sdk.so", "src/lib/libclob_sdk.dylib"];
            #[cfg(not(any(target_os = "windows", target_os = "macos")))]
            let candidates = ["lib/libclob_sdk.so", "src/lib/libclob_sdk.so"];
            candidates
                .into_iter()
        ...
        .context(
            "CLOB SDK not found. For sports trailing LIVE mode, place libclob_sdk in ./lib/ \
             (e.g. lib/libclob_sdk.so on Linux, lib/clob_sdk.dll on Windows). \
             Or set LIBCOB_SDK_SO to the full path. Build: cd polymarket-clob-sdk && cargo build --release --features clob",
        )?;
    let lib = LIB
